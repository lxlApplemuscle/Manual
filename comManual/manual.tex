...
\begin{document}
	...
	\section{GitHub}
		...
		\subsection{下载与上传文件}
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=.45\textwidth]{D:/professional courses/Computerese Manual/pic/git0}
				\caption{Git基本操作示意图}
				\label{pic2}
			\end{figure}
...
ssh -T git@github.com 
			...
			\begin{figure}[H]
				\centering
				\includegraphics[width=.55\textwidth]{D:/professional courses/Computerese Manual/pic/git2}
				\caption{成功完成连接展示}
				\label{pic4}
			\end{figure}
		\subsubsection{推送文件}
			...
			\begin{itemize}[leftmargin=12pt]
				\item[2]
				在执行\mybox[green]{clone}命令后，可以将远程仓库中的内容clone到本地仓库中，实现文件的下载：
				\begin{lstlisting}[language=bash, numbers=none]
git clone git@github.com:UserName/RepoName.git
			...
			\begin{itemize}[leftmargin=12pt]
				\item[3]
				向本地仓库中手动添加拟推送远程仓库的文件及文件夹\footnote[1]{注意：此处手动向本地仓库中添加文件并不意味着已上传至本地仓库中（可以看作你想存储的“货物”已经进了清单，但还没有入库）}，用\mybox[green]{ls}可以看到本地仓库中的文件，然后在本地仓库中执行\mybox[green]{add}命令，添加文件到暂存区。
				\begin{lstlisting}[language=bash, numbers=none]
...
git status            #查看仓库当前的状态，显示有变更的文件
			...
			\begin{itemize}[leftmargin=12pt]
				\item[4]
				文件进入暂存区后，执行\mybox[green]{commit}命令提交到本地仓库，最后执行\mybox[green]{push}命令，将文件推送至远程仓库中，完成文件的上传。
				\begin{lstlisting}[language=bash, numbers=none]
